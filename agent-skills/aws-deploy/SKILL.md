...
- **Submit jobs:** Source secrets in the same invocation: `source deploy/set-deploy-secrets.sh && ./deploy/scripts/submit-jobs/submit-transcribe-job.sh ...` (or `submit-transcript-postprocess-job.sh` for CPU queue, or `submit-llm-analysis-job.sh` for GPU). Transcript postprocess uses the **CPU queue**; transcribe and LLM analysis use the GPU queue. By default, list jobs in S3 first (bucket from `source deploy/set-deploy-secrets.sh && cd deploy/terraform && terraform output -raw s3_bucket_name`). Run from repo root; scripts resolve Terraform outputs from `deploy/terraform/`.
