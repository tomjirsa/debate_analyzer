...
- **Job 4 (LLM analysis):** Optional. After transcribe, run LLM analysis on transcript(s) to get main topics, per-topic summaries, and per-speaker contributions. Uses a **dedicated LLM image** (see [LLM_ANALYSIS.md](LLM_ANALYSIS.md)). Use `submit-llm-analysis-job.sh`.
