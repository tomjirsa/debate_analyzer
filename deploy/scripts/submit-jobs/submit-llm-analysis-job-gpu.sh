#!/usr/bin/env bash
# Submit an AWS Batch job to run LLM analysis on a transcript on GPU (Job 4 GPU).
# Usage: ./submit-llm-analysis-job-gpu.sh <transcript_s3_uri>
# Example: ./submit-llm-analysis-job-gpu.sh s3://bucket/jobs/job-id-123/transcripts/foo_transcription.json
# Or:     ./submit-llm-analysis-job-gpu.sh s3://bucket/jobs/job-id-123/transcripts  (processes all *_transcription.json under prefix)
# Job runs on the GPU queue. Requires the LLM GPU image to be built and pushed to the debate-analyzer-llm ECR repo (e.g. tag latest-gpu).
...
echo "Submitting LLM analysis job (GPU): $JOB_NAME"
